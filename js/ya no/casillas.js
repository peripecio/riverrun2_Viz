...
//		console.log("INIT FONTS: " + i + " --- " + fonts[i-1]['a'] + " ++++ " + fonts[i-1]['b']);
	...
	// Leera con un timer
	// . . . 
	tid = setInterval(timerUpdate, 10000);
	...
	// si click en casilla: renovar + hacer comparacion con las otras
	$('.casilla').click(function(e) {
		var idCasillaTmp = $(this).parent().attr("data-id");
		renovarCasilla(idCasillaTmp);
		e.preventDefault();
	});
...
function timerUpdate() {
	...
	// Esto para actualizar la comprobacion de posiciones (TEST)
	$(".wRep").mouseover(function() {
		// TEST
...
function crearCasillas() {
	...
	// crear casillas en la obra: array casillas
...
function renovarCasilla(idCasillaTmp) {
	...
	$.ajax({
        type: "POST",
        url: "http://localhost:8888/php_markov/genera_json.php", //?nCasilla=5",
        dataType: 'html',
        data: { 	// Esto es absurdo pero prepara la respuesta json para varios ids de casillas
                nCasilla: idCasillaTmp 
              },
        success: function(data){
		    var json = eval("(" + data + ")");

		    txtGen = json.txtGenerado;
...
//				console.log(palabra + " *** Ya tiene <SPAN> ******");
		...
		if(!bSpan) {
			...
			var clase = "wRep txta " + fonts[fontAct]['b'] + " txtRojo fontSzUp";
...
function scrollToCasillas(id){
    $('html,body').animate({scrollTop: $('.contCasilla[data-id|="'+id+'"]').offset().top - 150},'slow');
//    $('.contCasilla[data-id|="'+id+'"]').css('border','1px dotted green');
